...
// Values are wall clock in the user's timezone, never instants, and are parsed
// only through the time module.
...
/** Must match the bound in `features/settings/schemas.ts`. */
const MAX_WINDOWS = 6;
...
/**
 * The window "Add" would append, or null when the day has no room. Placed
 * after the latest end so the schema never merges it into a neighbour on save.
 */
...
/**
 * Commits on blur or Enter, not per keystroke: for a time input `"1"` on the
 * way to `"13:00"` is `01:00`. A backwards window is held, not sent: moving a
 * window later means editing the start before the end.
 */
function WindowRow({
  id,
  label,
  ...
  // the reset drafts, so `onBlur` still holds the abandoned text. A ref is what
